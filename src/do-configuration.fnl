...
(__.module
  ...
  (fn nearest-n11 [c]
    ...
    (var min nil)
    (each [___ name (ipairs n11-names)]
          (if (_.nil? min)
	      (set min {: name :dist (c-dist name)})
  ...
  (fn reset-theme []
    ...
    (fn h-of [name]
      (h-add Thm.root-n11-h
             (- (. h-by-norm11-name name)
                (. h-by-norm11-name Thm.root-n11-name))))
  ...
  (fn setup-completion []
    ...
    (-> {:snippet {:expand #(__.|| vim.fn "vsnip#anonymous" & $1.body)}
         :window {}
  ...
  (fn setup-paredit []
    (setup-plugin :nvim-paredit
                  {:use_default_keys true :indent {:enabled true}}))
  ...
  (fn setup-ibl []
    ...
    (fn set-hl []
      (doto vim.api
        (__.|| :nvim_set_hl & 0 :iblz1 (hl :cyan))
        (__.|| :nvim_set_hl & 0 :iblz2 (hl :amber))
        (__.|| :nvim_set_hl & 0 :iblz3 (hl :magenta))
        (__.|| :nvim_set_hl & 0 :iblz4 (hl :green))
        (__.|| :nvim_set_hl & 0 :iblz5 (hl :orange))
        (__.|| :nvim_set_hl & 0 :iblz6 (hl :indigo))
        (__.|| :nvim_set_hl & 0 :iblz7 (hl :lime))
        (__.|| :nvim_set_hl & 0 :iblz8 (hl :red))))

    (hooks.register hooks.type.HIGHLIGHT_SETUP set-hl)
    (set-hl)
    (loc hl-list [:iblz1 :iblz2 :iblz3 :iblz4 :iblz5 :iblz6 :iblz7 :iblz8])
